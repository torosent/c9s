...
c9s supports k9s-style plugins — short shell snippets bound to a hotkey
within a specific resource scope. Plugins live in
`~/.config/c9s/plugins/` as TOML files (one plugin per file or grouped).
...
| `scope` | yes | Resource type the shortcut applies to (`containers`, `images`, `volumes`, ...). |
| `shortcut` | yes | Key sequence (e.g. `Shift-L`, `Ctrl-T`). |
| `description` | yes | Short blurb shown in the help overlay. |
| `command` | yes | Shell command. Variables: `$RESOURCE_NAME`, `$RESOURCE_ID`, `$RESOURCE_IMAGE`. |
| `confirm` | no | If `true`, ask for confirmation before running. |
| `background` | no | If `true`, fork without suspending; otherwise suspend-and-shell-out. |

### Substitution variables

When the plugin fires on a focused row, c9s substitutes:

- `$RESOURCE_ID` — the row's primary ID (e.g. container UUID).
- `$RESOURCE_NAME` — the row's display name.
- `$RESOURCE_IMAGE` — for container/image rows, the image reference.

### Loading

c9s loads all `*.toml` files in `~/.config/c9s/plugins/` at startup and on
config reload (fsnotify). Errors in individual plugin files are logged
...
command = "stern $RESOURCE_NAME --tail 100"
```

After saving this file, focus a container row and press `Shift+L` —
c9s suspends, runs `stern <name> --tail 100`, and resumes when you exit.
